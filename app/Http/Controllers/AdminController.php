...
class AdminController extends Controller
{
    public function index()
    {
        ...
        $monthlyDatas = DB::select("SELECT M.id As MonthNo, M.name As MonthName,
                                    ...
                                    sum(if(status='delivered',total,0)) As TotalDeliveredAmount,
                                    sum(if(status='canceled',total,0)) As TotalCanceledAmount
                                    From Orders WHERE YEAR(created_at)=YEAR(NOW()) GROUP BY YEAR(created_at), MONTH(created_at), 			  
    ...
    public function brand_delete($id)
    {
        ...
        $brand->delete();
        return redirect()->route('admin.brands')->with('status', 'Bran has been deleted successfully');
    }
    ...
    public function GenerateSlideThumbailsImage($image, $imageName)
    {
        ...
        $img = Image::read($image->path());
        $img->cover(690, 690, "top");
        $img->resize(690, 690, function ($constraint) {
            $constraint->aspectRatio();
        })->save($destinationPath . '/' . $imageName);
    }
